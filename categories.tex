...
A \textit{category} $\mathcal{C}$ is an algebraic structure consisting of
\In{enumerate}{
...
\item \textit{Idenitities} or \textit{units}: a family of morphisms:
    \In{align*}{
        (\operatorname{id} _A : \mathcal{C}(A,A))_{A \in \operatorname{Ob}\mathcal{C}}
    }
}
...
we denote an object $A$ by a tagged string:
\In{align*}{
    \strdgathered{
...
Check this notation creates no ambiguity; thanks to the coherence.
