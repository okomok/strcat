...
\definitionof{Uniqueness}{
    ...
    using the third lambda-tasted form, meaning that %
...
\definitionof{Unique Existential Quantifier}{
    ...
    meaning that ``there exists x unique thing that satisfies $P$".
}
...
\In{remark}{
    ...
    states ``there exists a unique $x$ that satisfies $P$. Furthermore, the $x$ satisfies $Q$".
}
...
\definitionof{Functional Bijectivity}{
    Given a function $g : Y \to X$, the statement
    \In{align*}{
        (\exists f : X \to Y)(\forall x \in X)(\forall y \in Y)(x = g(y) \Leftrightarrow y = f(x))
    }
