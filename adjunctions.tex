...
A nice consequence is that the bijectivity needs no boxes, expressed by natural transformations only.
\In{align*}{
    \strdgathered{
        \strdmid
        ,\strdwidecell{1}{1}{f}
...
\In{align*}{
    & (\textstyle\lim_X : \text{lim}T \to TX)_X : \text{limiting cone} \\
    \implies & (G(\textstyle\lim_X) : G(\text{lim}T) \to G(TX))_X : \text{limiting cone}
}
\end{proposition}
